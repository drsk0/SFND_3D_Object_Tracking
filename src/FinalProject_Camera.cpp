...
int main(int argc, const char *argv[]) {
  ...
  int imgStartIndex = 0;
  // first file index to load (assumes Lidar and camera
  ...
  // no. of images which are held in memory (ring
  // buffer) at the same time
  vector<DataFrame> dataBuffer;
  // list of data frames which are held in memory
  // at the same time
  bool bVis = false;
  // visualize results
  ...
  for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex;
    ...
    // wait until at least two images have been processed
    {
      ...
      // cout << "match bounding boxes time: " << 1000 * t / 1.0 << " ms" <<
      // endl;
      ...
      for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin();
        ...
        for (auto it2 = (dataBuffer.end() - 1)->boundingBoxes.begin();
          ...
          // check wether current match partner
        ...
        // only compute TTC if we have Lidar points
        {
          ...
          bVis = true;
          if (bVis) {
            cv::Mat cameraImg = (dataBuffer.end() - 1)->cameraImg;
            cv::Mat visImg = (dataBuffer.end() - 1)->cameraImg.clone();
            // showLidarImgOverlay(visImg, currBB->lidarPoints, P_rect_00,
            // R_rect_00, RT, &visImg); cout << "nr keypoints: " <<
            // currBB->keypoints.size() << endl; cout << "nr matches: " <<
            // currBB->kptMatches.size() << endl; cv::drawKeypoints(cameraImg,
            // currBB->keypoints, visImg, cv::Scalar::all(-1),
            // cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS); cv::rectangle(visImg,
            // cv::Point(currBB->roi.x, currBB->roi.y), cv::Point(currBB->roi.x
            // + currBB->roi.width, currBB->roi.y + currBB->roi.height),
            // cv::Scalar(0, 255, 0), 2); cv::rectangle(visImg,
            // cv::Point(prevBB->roi.x, prevBB->roi.y), cv::Point(prevBB->roi.x
            // + prevBB->roi.width, prevBB->roi.y + prevBB->roi.height),
            // cv::Scalar(0, 0, 255), 2);
